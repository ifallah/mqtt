...
- (IBAction)connectPressed:(id)sender {
    ...
    if ([[[self connectButton] currentTitle]  isEqual:@"Connect"]) {
        ...
        NSArray *servers = [ConnectViewController parseCommaList:servmqtt];
                            //self.serverInput.text];
        NSArray *ports = [ConnectViewController parseCommaList:portmqtt];
                          //self.portInput.text];
...
-(void)sendetomqtt{
    ...
    NSString *topic = @"hi";
    NSString *payload = @"aim iphone :)";
    //NSString *payload = [[NSString alloc]initWithData:<#(nonnull NSData *)#> encoding:<#(NSStringEncoding)#>
    
    //initWithBytes:msg.payload length:msg.payloadLength encoding:NSUTF8StringEncoding];
    
    [[Messenger sharedMessenger] publish:topic payload:payload qos:0 retained:0];
